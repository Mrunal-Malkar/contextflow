...
function CanvasContent() {
  ...
  const baseNodes: Node[] = useMemo(() => {
    ...
    const selectedGroup = selectedGroupId
      ? project.groups.find((g) => g.id === selectedGroupId)
      : null

    // Find connected contexts for selected user (via user needs)
    const selectedUserNeedConnectionsForUser = selectedUserId
      ? project.userNeedConnections?.filter(
          (uc: UserNeedConnection) => uc.userId === selectedUserId
        ) || []
      : []
    const connectedUserNeedIds = new Set(
      selectedUserNeedConnectionsForUser.map((uc: UserNeedConnection) => uc.userNeedId)
    )
    ...
    const relationshipConnectedContextIds = new Set(
      selectedRelationship
        ? [selectedRelationship.fromContextId, selectedRelationship.toContextId]
        : []
    )
    ...
    const hoverConnectedContextIds = getHoverConnectedContextIds(
      hoveredContextId,
      ...
      viewMode !== 'distillation' && project.userNeeds
        ? project.userNeeds
            ...
            .map((userNeed) => {
              ...
              const isHighlightedByConnection =
                userNeed.id === userNeedConnectionUserNeedId ||
                userNeed.id === needContextConnectionUserNeedId
  ...
  const onNodesChange = useCallback(
    (changes: any[]) => {
      ...
      const isReactFlowMultiDrag =
        positionChanges.length > 1 && positionChanges.length === allSelectedIds.length
      ...
      {invalidConnectionAttempt && (
        <ConnectionGuidanceTooltip
          ...
          onCreateUserNeed={() => {
            ...
            if (
              invalidConnectionAttempt.sourceType === 'user' &&
              invalidConnectionAttempt.targetType === 'context'
            ) {
